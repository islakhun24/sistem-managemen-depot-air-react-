...
const Index = () => {
  ...
  const [loading, setLoading] = useState(true);
    ...
    const fetchData = useCallback(async () => {
      const response = await service.getCustomer(query);
      return response;
    }, [query]);
              ...
              {customer.map((val) => {
                return (
                  ...
                  <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                      <p className="text-gray-900 whitespace-no-wrap">{val.nama_customer}</p>
                    </td>
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                      <p className="text-gray-900 whitespace-no-wrap">{val.nohp_customer}</p>
                    </td>
                    <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                      <p className="text-gray-900 whitespace-no-wrap">{val.alamat_customer}</p>
                    </td>
        ...
        <Dialog
            ...
            >
                ...
                <DialogContentText id="alert-dialog-description">
                    apakah anda yakin ingin Hapus customer ?
                </DialogContentText>
                </DialogContent>
                <DialogActions>
